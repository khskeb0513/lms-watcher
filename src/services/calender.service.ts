...
export class CalenderService {
  ...
  public async getCalender() {
    ...
    return $('div.schedule-Detail-Box').toArray().map(v => {
  ...
  public async getReportCalender() {
    ...
    const $ = cheerio.load(body);
    return $('a.site-link').toArray().map(v => {
      const detailArr = $(v).children().toArray().map(v => {
        return $(v).text();
      });
      ...
      const parseDate = (str: string) => {
        const onlyNumber = [...str].map(v => parseInt(v, 10)).filter(v => !!v || v === 0).join('');
        if (onlyNumber.length === 0) return null;
        else {
          if (onlyNumber.length === 12) {
            const date = moment.utc(onlyNumber, 'YYYYMMDDHHmmss');
            return str.includes('오전')
              ? date
              : date.add(12, 'hours');
          } else {
            const date = moment.utc(onlyNumber.slice(0, 8) + '0' + onlyNumber.slice(8, 11), 'YYYYMMDDHHmm');
            return str.includes('오전')
              ? date
              : date.add(12, 'hours');
          }
      ...
      return {
        ...
        courseTitle: String(detailArr[0])
          .replace(/\n/ig, '').replace(/ {2}/g, ''),
        endDate, lateEndDate, submitStatus,
      };
